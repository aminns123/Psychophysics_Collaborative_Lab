...
class LegacyStaircaseSession:
    def __init__(
        ...
        *,
        ...
        self.metadata_path = (
            ...
            else Path(response_path).with_suffix(".session.json")
        )
        self.conditions_path = conditions_path
        self.trial_log = TrialLog(
            Path(trial_log_path)
            if trial_log_path is not None
            else Path(response_path).parent / "trials.tsv"
        )
    ...
    def _portable_trial_log_reference(self):
        try:
            return self.trial_log.path.resolve().relative_to(
                self.metadata_path.parent.resolve()
            ).as_posix()
        except ValueError:
            return self.trial_log.path.name
    ...
    def save(self, status=None, error=None):
        ...
        metadata.update({
            ...
            "reversal_basis": (
                "recorded_post_update_screen_intensity; flats ignored; "
                "no placeholder row"
            ),
            "canonical_trial_log": self._portable_trial_log_reference(),
            "path_policy": "canonical file references are relative to adaptive_session.json",
    ...
    def _capture_pre_response(self, event, trial, args):
        ...
        presented_intensity = float(
            conditions["staircase_intensity_active"][staircase_id]
        )
        ...
        if target == response:
            expected_step = (
                "down" if state.streak.correct + 1 == config.n_down else "hold"
            )
        else:
            expected_step = (
                "up" if state.streak.incorrect + 1 == config.n_up else "hold"
            )
        ...
        def log(event, time, trial, args):
            ...
            try:
                ...
                if self.run.total_trials != previous_count:
                    if before is None:
                        raise RuntimeError(
                            "Adaptive response count changed without a canonical "
